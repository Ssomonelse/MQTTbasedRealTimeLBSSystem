...
function initialize() {
	...
	myDate = $('#startdate').datepicker({
		...
		onSelect : function(fd, d, picker) {
			if(!d) return;
			preDate = d;
			myDate2.update('minDate', preDate);
		}
	}).data('datepicker');
...
function getLog() {
	...
	toggleFlag = 1;
	var date1 = myDate.selectedDates;
	var date2 = myDate2.selectedDates;
	console.log(date1+"\n"+date2);
	/* var start, end;
	...
	$.ajax({
		type:"POST",
		url:"http://localhost:8080/WebServer/DataController?type=tracking&pubmsg=" + msg,
		data: msg,
		dataType:"json",
		success:function(data){
	...
	myDate.clear();
	myDate2.clear();
	myDate.update('minDate', "");
	myDate.update('maxDate', "");
	myDate2.update('minDate', "");
	myDate2.update('maxDate', "");
}
k=0;
